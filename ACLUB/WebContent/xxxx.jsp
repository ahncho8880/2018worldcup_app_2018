...
<body>
	<nav class="navbar navbar-expand-lg navbar-light by light">
		<a class="navbar-brand" href="Board.jsp">A클럽 홈페이지</a> <a
    ...
    %>
			<tr>
				<th scope="row"><%=team.get(i).getHome() %> vs <%=team.get(i).getAway() %></th>
				<%
      ...
      for(int j=0;j<id.size();j++){
    	  ...
    	  %>
				<td><%=list.get(0).getTeam() %> <%=list.get(0).getWinScore() %>
					: <%=list.get(0).getLoseScore() %></td>
				<%
  		}catch(Exception e){
